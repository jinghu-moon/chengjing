...
export function useBookmarks() {
   ...
   * 编辑书签标题
   */
  async function editBookmarkTitle(index: number, newTitle: string) {
    if (!store.value) return
    const id = store.value.ids[index]

    try {
      if (isExtension.value) {
        await chrome.bookmarks.update(id, { title: newTitle })
      } else {
        console.log('[Mock] Edit title:', id, newTitle)
      }
      await loadBookmarks()
    } catch (e) {
      console.error('Failed to edit title:', e)
    }
  }
   ...
   * 编辑书签 URL
   */
  async function editBookmarkUrl(index: number, newUrl: string) {
    if (!store.value) return
    const id = store.value.ids[index]
    ...
    try {
      if (isExtension.value) {
        ...
        console.log('[Mock] Edit URL:', id, newUrl)
      }
      await loadBookmarks()
    } catch (e) {
  ...
  async function deleteBookmark(index: number) {
    if (!store.value) return
    const id = store.value.ids[index]
    const isFolder = store.value.isFolder[index] === 1
   ...
   * 复制链接到剪贴板
   */
  async function copyBookmarkUrl(index: number) {
    if (!store.value) return
    const url = store.value.urls[index]

    try {
      await navigator.clipboard.writeText(url)
      console.log('[Copy] URL copied:', url)
    } catch (e) {
