...
const classes = computed(() => [
  ...
  `lime-btn-group--shape-${props.shape}`,
  {
    ...
    'lime-btn-group--split': props.split,
  },
])
...
/* Split Button Mode */
.lime-btn-group--split :deep(.lime-btn:not(:last-child))::after {
  content: '';
  position: absolute;
  right: 0;
  top: 20%;
...
.lime-btn-group--split:not(.lime-btn-group--has-gap) :deep(.lime-btn:not(:first-child)) {
  margin-left: 0;
}
</style>
