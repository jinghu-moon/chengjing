...
const props = withDefaults(defineProps<ButtonProps>(), {
  ...
  iconPlacement: 'left',
  keyboard: buttonConfig.defaultKeyboard,
  loading: false,
  tag: 'button',
  type: 'button',
})
...
const handleKeyDown = (e: KeyboardEvent) => {
  // 原生 button 元素已支持键盘操作，无需额外处理
  if (isButton.value) return

  // 仅为非 button 标签（如 div、span、a）添加键盘支持
  if (!props.keyboard || isDisabled.value) return
