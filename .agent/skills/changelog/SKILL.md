---
name: changelog
description: Track feature-level changes during development as structured log entries for later commit reference. Triggers on "记录变更", "记一下", "log", "changelog", "变更日志". Supports view, merge, and clear operations. Auto-consulted when git-commit skill triggers.
---
...
- Scope: follow git-commit skill's Scope Mapping
...
When git-commit skill triggers and `change-log.txt` has entries:
...
1. Prompt user: "Changelog detected. Use it for commit message? (yes / no / skip)"
   ...
   - **no**: ignore log, proceed with normal git-commit flow
